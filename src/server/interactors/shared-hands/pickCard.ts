...
> => {
  ...
  const pickResult = findResult.value.toPicking({
    index: param.index,
    amount: param.amount,
  ...
  const saveResult = param.implementationContainer.handTelepresenceRepository.save(
    pickResult.value.sharedHand,
  );
  if (saveResult instanceof Failure) {
    return saveResult;
  }

  return new Success({ handTelepresence: pickResult.value.sharedHand });
};
